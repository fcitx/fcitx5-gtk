...
static GDBusInterfaceInfo *_fcitx_client_get_clientic_info(void) {
    ...
    if (g_once_init_enter(&has_info)) {
        GDBusNodeInfo *introspection_data;
        introspection_data =
            g_dbus_node_info_new_for_xml(ic_introspection_xml, NULL);
        info = introspection_data->interfaces[0];
        g_once_init_leave(&has_info, 1);
    }
    return info;
...
void fcitx_client_set_capability(FcitxClient *self, guint64 flags) {
    ...
    if (self->priv->icproxy) {
        g_dbus_proxy_call(self->priv->icproxy, "SetCapability",
                          g_variant_new("(t)", iflags), G_DBUS_CALL_FLAGS_NONE,
                          -1, NULL, NULL, NULL);
    }
}
...
void _fcitx_client_process_key_cb(G_GNUC_UNUSED GObject *source_object,
                                  GAsyncResult *res, gpointer user_data) {
...
void _fcitx_client_process_key_cancelled(
...
void fcitx_client_process_key(FcitxClient *self, guint32 keyval,
                              ...
                              gboolean isRelease, guint32 t, gint timeout_msec,
                              GCancellable *cancellable,
                              GAsyncReadyCallback callback,
                              gpointer user_data) {
    if (self->priv->icproxy) {
 ...
 */
FCITXGCLIENT_EXPORT
gboolean fcitx_client_process_key_sync(FcitxClient *self, guint32 keyval,
                                       guint32 keycode, guint32 state,
...
static void _fcitx_client_create_ic(G_GNUC_UNUSED FcitxConnection *connection,
                                    gpointer user_data) {
    ...
    g_dbus_proxy_new(
        ...
        _fcitx_client_get_interface_info(), "org.fcitx.Fcitx5", "/inputmethod",
        "org.fcitx.Fcitx.InputMethod1", self->priv->cancellable,
        _fcitx_client_create_ic_phase1_finished, self);
}

static void _fcitx_client_disconnect(G_GNUC_UNUSED FcitxConnection *connection,
                                     gpointer user_data) {
    FcitxClient *self = user_data;
    _fcitx_client_clean_up(self, FALSE);
}
...
static void
_fcitx_client_create_ic_phase1_finished(G_GNUC_UNUSED GObject *source_object,
                                        GAsyncResult *res, gpointer user_data) {
...
static void _fcitx_client_create_ic_cb(GObject *source_object,
    ...
    path = g_variant_get_string(pathVariant, NULL);
    ...
    g_dbus_proxy_new(
        ...
        "org.fcitx.Fcitx5", self->priv->icname, "org.fcitx.Fcitx.InputContext1",
        self->priv->cancellable, _fcitx_client_create_ic_phase2_finished, self);
}

static void
_fcitx_client_create_ic_phase2_finished(G_GNUC_UNUSED GObject *source_object,
                                        GAsyncResult *res, gpointer user_data) {
    g_return_if_fail(user_data != NULL);
    g_return_if_fail(FCITX_IS_CLIENT(user_data));
    FcitxClient *self = (FcitxClient *)user_data;
...
static void _fcitx_client_g_signal(G_GNUC_UNUSED GDBusProxy *proxy,
