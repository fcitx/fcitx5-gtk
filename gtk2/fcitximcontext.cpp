...
fcitx_im_context_class_init(FcitxIMContextClass *klass)
{
    ...
    _use_key_snooper = !get_boolean_env ("IBUS_DISABLE_SNOOPER", !(_ENABLE_SNOOPER))
    ...
    if (_use_key_snooper) {
        ...
        for (p = apps; *p != NULL; p++) {
            if (g_regex_match_simple (*p, prgname, (GRegexCompileFlags) 0, (GRegexMatchFlags) 0)) {
...
fcitx_im_context_init(FcitxIMContext *context)
{
    ...
    context->last_updated_capacity = (guint64) fcitx::CapabilityFlag::SurroundingText;
                                 ...
                                 GdkEventKey  *event)
{
    ...
    if (fcitx_client_is_valid(fcitxcontext->client) && fcitxcontext->has_focus) {
        ...
        if (_use_sync_mode) {
            gboolean ret = fcitx_client_process_key_sync(fcitxcontext->client,
                                                    event->keyval,
                                  ...
                                  gpointer user_data)
{
    ...
    gboolean ret = fcitx_client_process_key_finish(FCITX_CLIENT(source_object), res);
    if (!ret) {
        event->state |= (guint32) fcitx::KeyState::IgnoredMask;
                                              ...
                                              int cursor_pos, void* user_data)
{
    ...
    for (i = 0;i < array->len;i++) {
        ...
        FcitxPreeditItem *preedit = (FcitxPreeditItem *) g_ptr_array_index(array, i);
                                  ...
                                  gint cursor_index)
{
    ...
    if (fcitx_client_is_valid(fcitxcontext->client) &&
        !(fcitxcontext->last_updated_capacity & (guint64) fcitx::CapabilityFlag::Password)) {
        gint cursor_pos;
        guint utf8_len;
        gchar *p;
...
_fcitx_im_context_set_capacity(FcitxIMContext* fcitxcontext, gboolean force)
{
    if (fcitx_client_is_valid(fcitxcontext->client)) {
        ...
        if (fcitxcontext->use_preedit) {
            flags |= (guint64) fcitx::CapabilityFlag::Preedit | (guint64) fcitx::CapabilityFlag::FormattedPreedit;
        }
        ...
        if (G_UNLIKELY(update || force))
            fcitx_client_set_capability(fcitxcontext->client, fcitxcontext->last_updated_capacity);
    }
}
...
void _fcitx_im_context_connect_cb(FcitxClient* im, void* user_data)
{
    ...
    if (display) {
        send_uuid_to_x11(display, fcitx_client_get_uuid(im));
    }
...
_key_snooper_cb(GtkWidget *widget, GdkEventKey *event, gpointer user_data)
{
    ...
    do {
        ...
        if (_use_sync_mode) {
            ...
            retval = fcitx_client_process_key_sync(fcitxcontext->client,
                                                    event->keyval,
                                                    event->hardware_keycode,
                                                    event->state,
...
#define CASE_PURPOSE(_PURPOSE, _CAPACITY) \
    ...
    switch(purpose) {
        ...
        CASE_PURPOSE(GTK_INPUT_PURPOSE_PASSWORD, fcitx::CapabilityFlag::Password)
        CASE_PURPOSE(GTK_INPUT_PURPOSE_PIN, (guint64) fcitx::CapabilityFlag::Password | (guint64) fcitx::CapabilityFlag::Digit)
...
#define CHECK_HINTS(_HINTS, _CAPACITY) \
    ...
    CHECK_HINTS(GTK_INPUT_HINT_NO_SPELLCHECK, fcitx::CapabilityFlag::NoSpellCheck);
    CHECK_HINTS(GTK_INPUT_HINT_WORD_COMPLETION, fcitx::CapabilityFlag::WordCompletion)
    CHECK_HINTS(GTK_INPUT_HINT_LOWERCASE, fcitx::CapabilityFlag::Lowercase)
    CHECK_HINTS(GTK_INPUT_HINT_UPPERCASE_CHARS, fcitx::CapabilityFlag::Uppercase)
    CHECK_HINTS(GTK_INPUT_HINT_UPPERCASE_WORDS, fcitx::CapabilityFlag::UppercaseWords)
    CHECK_HINTS(GTK_INPUT_HINT_UPPERCASE_SENTENCES, fcitx::CapabilityFlag::UppwercaseSentences)
    CHECK_HINTS(GTK_INPUT_HINT_INHIBIT_OSK, fcitx::CapabilityFlag::NoOnScreenKeyboard)
