...
static void fcitx_im_context_class_init(FcitxIMContextClass *klass) {
  ...
  if (g_getenv("IBUS_ENABLE_SYNC_MODE") || g_getenv("FCITX_ENABLE_SYNC_MODE")) {
...
static void fcitx_im_context_init(FcitxIMContext *context) {
  ...
  g_signal_connect(context, "notify::input-hints",
                   G_CALLBACK(_fcitx_im_context_input_hints_changed_cb), NULL);
  ...
  if (fcitx_g_client_is_valid(fcitxcontext->client) &&
    ...
    if (_use_sync_mode) {
      ...
      fcitx_g_client_process_key(
          fcitxcontext->client, event->keyval, event->hardware_keycode,
          event->state, (event->type != GDK_KEY_PRESS), event->time, -1, NULL,
          _fcitx_im_context_process_key_cb, gdk_event_copy((GdkEvent *)event));
      event->state |= (guint32)fcitx::KeyState::HandledMask;
      return TRUE;
    }
  } else {
  ...
  for (i = 0; i < array->len; i++) {
    ...
    if (type & (guint32)fcitx::TextFormatFlag::HighLight) {
      ...
      if (context->client_window) {
        GtkWidget *widget;
        gdk_window_get_user_data(context->client_window, (gpointer *)&widget);
...
static void fcitx_im_context_set_surrounding(GtkIMContext *context,
  ...
  if (fcitx_g_client_is_valid(fcitxcontext->client) &&
    ...
    if (p || fcitxcontext->last_cursor_pos != cursor_pos ||
      ...
      fcitxcontext->last_anchor_pos = anchor_pos;
      fcitx_g_client_set_surrounding_text(fcitxcontext->client, p, cursor_pos,
                                          anchor_pos);
    }
  }
...
static GdkEventKey *_create_gdk_event(FcitxIMContext *fcitxcontext,
  ...
  GdkEventKey *event =
      (GdkEventKey *)gdk_event_new(isRelease ? GDK_KEY_RELEASE : GDK_KEY_PRESS);
...
static void _request_surrounding_text(FcitxIMContext **context) {
  if (*context && fcitx_g_client_is_valid((*context)->client) &&
    ...
    g_signal_emit(*context, _signal_retrieve_surrounding_id, 0, &return_value);
    if (*context)
      g_object_remove_weak_pointer((GObject *)*context, (gpointer *)context);
    else
      return;
    if (return_value) {
...
static gint _key_snooper_cb(GtkWidget *widget, GdkEventKey *event,
  ...
  do {
    ...
    if (_use_sync_mode) {
      ...
      fcitx_g_client_process_key(
          fcitxcontext->client, event->keyval, event->hardware_keycode,
          event->state, (event->type == GDK_KEY_RELEASE), event->time, -1, NULL,
          _fcitx_im_context_process_key_cb, gdk_event_copy((GdkEvent *)event));
      retval = TRUE;
    }
  } while (0);
